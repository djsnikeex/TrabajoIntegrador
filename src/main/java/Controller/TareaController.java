...
public class TareaController {
    ...
    public ResponseEntity <?> save(@RequestBody Tarea entity){
        try{
            return ResponseEntity.status(HttpStatus.OK).body(TareaService.save(entity));
        }
        catch(Exception e){
    ...
    @PutMapping("/{id}")
    public ResponseEntity <?> update(@PathVariable Long id, @RequestBody Tarea entity){
        try{
            return ResponseEntity.status(HttpStatus.OK).body(TareaService.update(id,entity));
        }
        catch(Exception e){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("{\"error\":\"Error, por favor intente mas tarde. \"}");
        }
